...
function DialogOverlay({ className, ...props }: React.ComponentPropsWithoutRef<typeof DialogPrimitive.Overlay>) {
  return (
    <DialogPrimitive.Overlay
      className={cn('fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-sm data-[state=open]:animate-in', className)}
      {...props}
    />
  )
}
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        className={cn(
          'fixed left-1/2 top-1/2 z-50 grid w-[calc(100%-2rem)] max-w-2xl max-h-[calc(100vh-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 overflow-hidden rounded-xl border bg-background p-0 shadow-lg duration-200 data-[state=open]:animate-in',
          className
        )}
        {...props}
      >
...
function DialogHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('flex flex-col space-y-1.5 border-b px-5 py-4 text-left', className)} {...props} />
}
